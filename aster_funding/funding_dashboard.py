#!/usr/bin/env python3
"""Simple web dashboard for Aster funding data stored in funding.db."""
...
<head>
  ...
  <title>Aster Funding Dashboard</title>
  <style>
    :root {{
      --bg: #0e172a;
...
<body>
  <div class="shell">
    <header>
      <div class="hero">
        <h1>Aster Funding Dashboard</h1>
        <div class="sub">查看 Aster 永续合约的基础信息、最新资金费率，以及过去 24 小时 / 3 天 / 7 天 / 15 天 / 30 天的累计资金费率。</div>
      </div>
  ...
  <script>
    ...
    function render() {{
      ...
      if (!sorted.length) {{
        body.innerHTML = '<tr><td colspan="99" class="dim">暂无数据，先运行 aster_funding_baseinfo.py 与 aster_funding_history.py</td></tr>';
        return;
      }}
