#!/usr/bin/env python3
"""Simple web dashboard for Hyperliquid funding data stored in funding.db."""
...
# Hyperliquid baseinfo 表中的 openInterest 字段，默认存“未平仓价值（USD）”
...
<head>
  ...
  <title>Hyperliquid Funding Dashboard</title>
  <style>
    :root {{
      --bg: #0e172a;
...
<body>
  <div class="shell">
    <header>
      <div class="hero">
        ...
        <div class="sub">查看 Hyperliquid 永续合约的基础信息、最新资金费率，以及过去 24 小时 / 3 天 / 7 天 / 15 天 / 30 天的累计资金费率。</div>
      </div>
    ...
    <div class="controls">
      ...
      <div class="range-row">
        <div>持仓量过滤 (百万 USD)：<span id="oiFilterLabel">0</span>+</div>
  ...
  <script>
    ...
    function updateMeta(payload) {{
      ...
      footer.textContent = '数据来源：allFunding/funding.db；资金费率为自然数值 (0.0001 = 0.01%)，累计为窗口内求和。';
    }}
    ...
    function render() {{
      ...
      if (!sorted.length) {{
        body.innerHTML = '<tr><td colspan="99" class="dim">暂无数据，先运行 hyperliquid_funding_baseinfo.py 与 hyperliquid_funding_history.py</td></tr>';
        return;
      }}
