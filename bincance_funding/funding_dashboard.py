#!/usr/bin/env python3
"""Simple web dashboard for Binance funding data stored in funding.db."""
...
def ensure_info_table(conn: sqlite3.Connection, table: str) -> None:
    """Create info table if missing so the dashboard can start empty."""
    conn.execute(
        f"""
...
def ensure_history_table(conn: sqlite3.Connection) -> None:
    """Create history table if missing so cumulative view does not fail."""
    conn.execute(
        f"""
...
def resolve_info_table(conn: sqlite3.Connection) -> str:
    """Pick existing base info table; prefer baseinfo, fallback to info; create primary if none."""
...
def build_payload() -> dict[str, Any]:
    ...
    for row in base_rows:
        sums = sums_by_symbol.get(
            row["symbol"], {key: 0.0 for key, _, _ in WINDOWS}
        )
...
def render_html() -> str:
    window_keys = json.dumps([key for key, _, _ in WINDOWS])
    return f"""<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Binance Funding Dashboard</title>
  <style>
    :root {{
      --bg: #0e172a;
...
<body>
  <div class="shell">
    <header>
      <div class="hero">
        <h1>Binance Funding Dashboard</h1>
        <div class="sub">快速查看当前永续合约的基础信息、最新资金费率，以及过去 24 小时 / 3 天 / 7 天 / 15 天 / 30 天的累计资金费率。</div>
      </div>
    ...
    <div class="controls">
      ...
      <div class="pill">
        ...
        <select id="sortKey">
          ...
          <option value="openInterest">未平仓合约数</option>
      ...
      <table>
        <thead>
          <tr id="thead-row">
            ...
            <th data-key="openInterestNotional" data-label="合约持仓量(USDT)">合约持仓量(USDT)</th>
            <th data-key="lastFundingRate" data-label="最新资金费率">最新资金费率</th>
            <th data-key="fundingIntervalHours" data-label="结算周期">结算周期</th>
  ...
  <script>
    ...
    const OI_THRESHOLDS = [0, 1, 3, 10, 30, 50, 100]; // million USDT
    ...
    function updateMeta(payload) {{
      ...
      footer.textContent = '数据来源：本地 funding.db；资金费率为自然数值 (0.0001 = 0.01%)，累计为窗口内求和。';
    }}
    ...
    function render() {{
      ...
      const filtered = dataCache.filter(item => {{
        const hitSymbol = !q || (item.symbol || '').includes(q);
      ...
      if (!sorted.length) {{
        body.innerHTML = '<tr><td colspan="99" class="dim">暂无数据，先运行 binance_funding_baseinfo.py 与 binance_funding_history.py</td></tr>';
        return;
      }}
