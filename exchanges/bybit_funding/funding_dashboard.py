#!/usr/bin/env python3
"""Simple web dashboard for Bybit funding data stored in funding.db."""
...
# Bybit baseinfo 表中的 openInterest 字段，默认存“未平仓价值（USDT）”
...
<head>
  ...
  <title>Bybit Funding Dashboard</title>
  <style>
    :root {{
      --bg: #0e172a;
...
<body>
  <div class="shell">
    <header>
      <div class="hero">
        <h1>Bybit Funding Dashboard</h1>
        <div class="sub">快速查看当前 USDT Linear 永续合约的基础信息、最新资金费率，以及过去 24 小时 / 3 天 / 7 天 / 15 天 / 30 天的累计资金费率。</div>
      </div>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th data-key="openInterestNotional" data-label="持仓(USDT)">持仓(百万USDT)</th>
            <th data-key="lastFundingRate" data-label="最新资金费率">最新资金费率 / APR</th>
  ...
  <script>
    ...
    function render() {{
      ...
      if (!sorted.length) {{
        body.innerHTML = '<tr><td colspan="99" class="dim">暂无数据，先运行 bybit_funding_baseinfo.py 与 bybit_funding_history.py</td></tr>';
        return;
      }}
