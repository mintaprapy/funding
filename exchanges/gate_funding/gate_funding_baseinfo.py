...
def main() -> None:
    ...
    with requests.Session() as session, sqlite3.connect(DB_PATH) as conn:
        ...
        collector_log_start("Gate", "base", detail=f"{len(symbols)} 个交易对")
        ...
        for symbol in symbols:
            ...
            rows.append(
                (
                    ...
                    insurance_balance,
                    to_plain_str((ticker or {}).get("volume_24h_base") or (ticker or {}).get("volume_24h")),
                    to_plain_str((ticker or {}).get("volume_24h_quote") or (ticker or {}).get("volume_24h_settle")),
