...
def main() -> None:
    ...
    with requests.Session() as session, sqlite3.connect(DB_PATH) as conn:
        ...
        collector_log_start("StandX", "base", detail=f"{len(symbols)} 个交易对")
        ...
        for idx, symbol in enumerate(symbols, 1):
            ...
            rows.append(
                (
                    ...
                    None,
                    to_plain_str(market.get("volume_24h")),
                    to_plain_str(market.get("volume_quote_24h")),
                    now_ms,
                )
            )
            if idx % 30 == 0 or idx == len(symbols):
                collector_log_progress("StandX", "base", detail="处理中", current=idx, total=len(symbols))
        ...
        if deleted:
            collector_log_progress("StandX", "base", detail=f"删除已下架交易对 {len(deleted)} 个")

        rows = stamp_rows_updated_at(rows)
        save_records(conn, rows)
        collector_log_end("StandX", "base", detail=f"入库 {len(rows)} 条到 {TABLE_NAME}")
