#!/usr/bin/env python3
"""Fetch StandX perp funding history into SQLite."""
...
def fetch_symbol_history(
    ...
    *,
    ...
    items.sort(key=lambda x: parse_iso_to_ms(str(x.get("time") or x.get("created_at") or "")) or 0)
    return items
...
def save_history(
    ...
    *,
    ...
    rows: list[tuple[Any, ...]] = []
    for item in records:
        funding_time = parse_iso_to_ms(str(item.get("time") or item.get("created_at") or ""))
        ...
        rows.append(
            (
                symbol,
                funding_time,
                to_plain_str(item.get("funding_rate") or item.get("fundingRate") or item.get("rate")),
                now_ms,
            )
        )
    if not rows:
...
def main() -> None:
    ...
    with sqlite3.connect(DB_PATH) as conn, requests.Session() as session:
        ...
        collector_log_start("StandX", "history", detail=f"{len(symbols)} 个交易对，近 {DAYS_TO_FETCH} 天资金费率")
        ...
        for idx, symbol in enumerate(symbols, 1):
            ...
            collector_log_progress("StandX", "history", detail=f"{symbol} 入库 {inserted} 条", current=idx, total=len(symbols))
    ...
    collector_log_end("StandX", "history")
