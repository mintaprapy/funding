...
def main() -> None:
    ...
    with requests.Session() as session, sqlite3.connect(DB_PATH) as conn:
        ...
        collector_log_start("Bitget", "base", detail=f"{len(symbols)} 个交易对")
        ...
        for symbol in symbols:
            ...
            rows.append(
                (
                    ...
                    None,
                    to_plain_str(ticker.get("baseVolume")),
                    to_plain_str(ticker.get("usdtVolume") or ticker.get("quoteVolume")),
                    now_ms,
                )
            )
