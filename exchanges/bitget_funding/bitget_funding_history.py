...
def main() -> None:
    ...
    with sqlite3.connect(DB_PATH) as conn, requests.Session() as session:
        ...
        collector_log_start("Bitget", "history", detail=f"{len(symbols)} 个交易对，近 {DAYS_TO_FETCH} 天资金费率")
        ...
        for idx, symbol in enumerate(symbols, 1):
            ...
            conn.commit()
            collector_log_progress("Bitget", "history", detail=f"{symbol} 入库 {inserted} 条", current=idx, total=len(symbols))

    collector_log_end("Bitget", "history")
